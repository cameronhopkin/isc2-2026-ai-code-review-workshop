...
- `.github/workflows/ci.yml`: on push and pull request, run on `ubuntu-latest`, `windows-latest`, and `macos-latest`. Set up Python 3.11 and 3.14, create a venv, install pinned requirements, run pytest for `reference-pipeline` and `target-app`, then run `smoke.py --dry-run`. No Ollama in CI in this phase.
